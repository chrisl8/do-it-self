...
3. Record the container in `installed-modules.yaml` under `modules.<module>.installed_containers`. `container-registry.yaml` is **not** modified — the catalog already ships with every container's schema.
...
4. Remove the container from `installed-modules.yaml` (`installed_containers` list). `container-registry.yaml` is **not** modified.
...
3. For each **installed** container in the module:
   ...
   - Preserve `icons/`, `images/`
   - Preserve `infisical/infisical-secrets.env` (the container's own durable
     bootstrap secrets — `generate-env.js` re-appends these into `.env` on
     every run; losing this file here left `.env` as the only copy, and the
     next full `.env` regen silently dropped the secrets and crashed the
     container)
4. Update the commit hash in `installed-modules.yaml`
5. Restart any running containers whose compose.yaml changed

Module updates do **not** modify `container-registry.yaml`. If a module's update introduces new or changed container schemas, the platform maintainer regenerates and commits the registry separately (see `regenerate-registry` below).
...
`web-admin` is a special case: its container definition (compose.yaml, tailscale-config) lives in the platform repo alongside its source code. It's treated as a platform container via a hardcoded allowlist in `scripts/lib/container-source.js` and is not part of any module.
...
- List of containers listed in `installed-modules.yaml` under `personal_containers`
...
~/containers/                        # platform git repo
  ...
  container-registry.yaml            # tracked catalog (merged view of all modules' schemas)
  user-config.yaml                   # user overrides (gitignored)
  installed-modules.yaml             # what's installed on this host (gitignored)
...
- `container-registry.yaml` is the catalog of every known container schema. Per-host install state (which module each container belongs to, plus `personal_containers`) lives in `installed-modules.yaml` — the registry carries no `source:` field.
...
- `web-admin` stays in the platform repo, identified as a platform container via a hardcoded allowlist in `scripts/lib/container-source.js`
