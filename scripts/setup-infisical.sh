...
# 2. Creates the Infisical .env file
...
INFISICAL_ENV="${INFISICAL_DIR}/.env"
if [[ ! -f "$INFISICAL_ENV" ]]; then
  ...
  cat > "$INFISICAL_ENV" << EOF
# Auto-generated by setup-infisical.sh
AUTH_SECRET=${AUTH_SECRET}
ENCRYPTION_KEY=${ENCRYPTION_KEY}
DB_PASSWORD=${DB_PASSWORD}
EOF
  chmod 600 "$INFISICAL_ENV"
  printf "${GREEN}Created %s${NC}\n" "$INFISICAL_ENV"
else
  printf "${GREEN}Infisical .env already exists, keeping it.${NC}\n"
  # Source existing values
  # shellcheck disable=SC1090
  source "$INFISICAL_ENV"
fi

# 2. Generate the container's .env with Tailscale credentials too
# The compose file needs TS_AUTHKEY and TS_DOMAIN from the user config
...
INFISICAL_ENV_CONTENT=$(cat "$INFISICAL_ENV")
# shellcheck disable=SC1090
source "$INFISICAL_ENV"
...
if [[ -f "$CRED_FILE" ]]; then
  ...
  PROJECT_OUTPUT=$(curl -sf "http://localhost:8080/api/v2/workspace" \
  ...
  if [[ -z "$PROJECT_ID" ]]; then
    ...
    curl -sf "http://localhost:8080/api/v2/folders" \
      -H "Authorization: Bearer ${IDENTITY_TOKEN}" \
      -H "Content-Type: application/json" \
...
echo "============================================"
echo ""
echo "Infisical web UI: https://infisical.$(grep TS_DOMAIN "${INFISICAL_DIR}/.env" 2>/dev/null | cut -d= -f2 || echo '<your-ts-domain>')"
...
echo "  3. Or use 'infisical secrets set' CLI to add secrets directly"
echo ""
