...
case "${SUBCOMMAND}" in
  ...
  help|--help|-h)
    ...
    echo "  update [<module>] [--container <name>[,<name>...]] [--no-restart] [--yes|-y]"
    echo "                                     Update module(s); by default only re-renders/"
    echo "                                     restarts containers whose files actually changed."
    echo "                                     A module.yaml change affects every installed"
    echo "                                     container from that module -- update aborts with"
    echo "                                     a warning in that case unless --yes is given, or"
    echo "                                     use --container to force just the one(s) you mean."
